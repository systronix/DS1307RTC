 ...
 * DS1307RTC.h - library for DS1307 RTC
  ...
  24 jul 2016 - systronix modified to use i2c_t3 library; create read_both();
 */
...
//
// systronix addition
// Acquire data from the RTC chip in BCD format; put it in our tmElements_t struct and convert that same time
// to a Unix time stamp so that the time stamp and struct elements are always in sync
//

bool DS1307RTC::get_time (tmElements_t &tm, time_t &timestamp)
	{
	if (read (tm))						// get register data into the struct
		{
		timestamp = (makeTime (tm));	// use the struct content to make the time stamp
		return true;
		}
